# Sub-cellular localisation: using it today

> Moved here from raven-toolbox's own documentation tree. Compartment assignment
> is still in development and not at parity with MATLAB RAVEN, so it is
> documented alongside its evidence base rather than on the user-facing site.
> See the [evidence base](README.md) for the measurements and design behind it.

`raven_toolbox.localization` assigns reactions to compartments by MILP — deterministic
...
1. **Load predictor/database scores** into the `gene × compartment`
   `raven_toolbox.localization.LocalizationScores` frame:
   `raven_toolbox.localization.load_deeploc` (DeepLoc 2 per-protein CSV),
   `raven_toolbox.localization.load_mulocdeep` (MULocDeep wide table),
   `raven_toolbox.localization.load_compartments` (COMPARTMENTS evidence database), or
   `raven_toolbox.localization.load_uniprot` (curated UniProtKB `Subcellular location`).
   `raven_toolbox.localization.fetch_uniprot_localization` queries the UniProt REST API by
   organism id directly, no manual export needed.
   Pass `raven_toolbox.localization.DEFAULT_COMPARTMENT_MAP` to rename predictor labels
   ...
   **Fuse and tune the evidence.** Since no single source is authoritative (two curated sources
   ...
   `raven_toolbox.localization.combine_scores` merges several `LocalizationScores` into a
...
2. **Predict / apply:** `raven_toolbox.localization.predict_localization` is the MILP
   ...
   `apply=False` you get a `raven_toolbox.localization.LocalizationProposal` diff to
   inspect before committing; `raven_toolbox.localization.apply_localization` applies a
   result.
3. **Triage for curation (optional):** `raven_toolbox.localization.triage_localization` takes
   the proposal + scores and returns a `raven_toolbox.localization.ReviewReport` ranking the
...
`raven_toolbox.localization.prepare_deeploc_input` writes that FASTA for your model's genes —
sequences fetched from UniProtKB, headers set to the **gene ids** so the predictor's output lines up
with the model (and with `raven_toolbox.localization.load_deeploc`) without remapping:
...
≥ 10 aa (enforced). `raven_toolbox.localization.fetch_protein_sequences` and
`raven_toolbox.localization.write_fasta` are the underlying building blocks if you need finer
