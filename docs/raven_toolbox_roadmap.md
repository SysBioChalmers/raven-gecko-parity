# raven-toolbox roadmap

> Moved here from raven-toolbox's own documentation tree, which was retired in
> favour of [raven-docs](https://raven-docs.readthedocs.io/). A plan for work in
> progress reads as a promise when it sits on a user-facing site; it belongs with
> the other development records instead.
...
know it is finished. [raven_toolbox_todo.md](raven_toolbox_todo.md) is the **backlog**: the item-level detail behind
each phase.
...
* Behind it, in this repo: extend [migration.md](https://raven-docs.readthedocs.io/en/latest/raven3-vs-raven-toolbox/) with the reverse direction
...
5. Then, as capacity allows: compartmentalisation, confidence tracking, ecModel handoff —
   see [todo.md](raven_toolbox_todo.md) §3.
...
* Resolve the open 💡 proposals in [improvements.md](https://github.com/SysBioChalmers/raven-toolbox/blob/develop/IMPROVEMENTS.md): implement or formally
  drop.
