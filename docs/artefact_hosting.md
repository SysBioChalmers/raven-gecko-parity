...
Where the large downloadable artefacts shared by RAVEN (MATLAB) and raven-toolbox
(Python) live, how they're versioned, and the exact commands to publish a new one. The
per-artefact build details are in [Maintaining KEGG data](maintaining_kegg_data.md) and
...
indexed by a single `manifest.json`. They are **not** attached to the `raven-toolbox` or
`RAVEN` code releases.
...
- One shared, versioned source of truth serves **both** raven-toolbox and MATLAB RAVEN
  --- the reason this decision is documented here rather than in either toolbox's own
  docs.
...
- **RAVEN (MATLAB)** `getKEGGModelForOrganism` downloads the HMM libraries from the
  matching `kegg<NNN>` release directly --- it has no baked registry to keep in sync;
  see [Data & binary manifest](data_manifest.md) for why that asymmetry is fine.
...
Everything is scripted and idempotent, run from a `raven-toolbox` checkout (there is
no MATLAB-side publishing tool: RAVEN only ever consumes these releases, never
produces them). Re-running skips assets already present, so a single-tool bump never
re-uploads the rest.
...
All four scripts are maintainer tooling in raven-toolbox:
[`build_binary_bundles.py`](https://github.com/SysBioChalmers/raven-toolbox/blob/develop/scripts/build_binary_bundles.py),
[`build_kegg_artefacts.py`](https://github.com/SysBioChalmers/raven-toolbox/blob/develop/scripts/README.md),
[`publish_to_raven_data.py`](https://github.com/SysBioChalmers/raven-toolbox/blob/develop/scripts/README.md),
[`make_registry_snippet.py`](https://github.com/SysBioChalmers/raven-toolbox/blob/develop/scripts/README.md).
...
registries in raven-toolbox's `data.py`/`binaries.py` --- change the manifest and run
