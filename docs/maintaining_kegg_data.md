...
How the published KEGG artefacts --- consumed by both raven-toolbox and MATLAB RAVEN
--- get rebuilt once per KEGG release. The build pipeline itself is implemented
entirely in raven-toolbox (Python); RAVEN MATLAB has no separate builder of its own
and only ever consumes the published output via `getKEGGModelForOrganism` (see
[Artefact hosting & publishing](artefact_hosting.md)). End users on either side
never do this --- they download the published, version-pinned artefacts. The build
has two implemented steps so far: **download**
(`reconstruction/kegg/download.py`) and **parse** (`reconstruction/kegg/parse.py`).
...
Build the per-domain profile-HMM libraries that the de-novo query path searches.
This needs **HMMER** (`hmmbuild`, `hmmpress`), **MAFFT**, and **CD-HIT** on `PATH`
(or set `RAVEN_PYTHON_HMMBUILD` / `RAVEN_PYTHON_MAFFT` / `RAVEN_PYTHON_CDHIT`,
etc.); install e.g. `conda install -c bioconda hmmer mafft cd-hit`.
...
for domain in ("prokaryotes", "eukaryotes"):
    build_hmm_library(
        organism_gene_ko,
        "keggdb/genes.pep",      # proteomes from step 1
        "keggdb/taxonomy",       # domain split, from step 1
        f"hmms/{domain}",
        domain=domain,
...
decompress it and search it directly with `hmmsearch`, which keeps the download
~10× smaller than a binary index, stays portable across HMMER versions (see
[maintaining_binaries.md](maintaining_binaries.md#native-windows-hmmer-332-from-raven-2105)),
and lets the same artefact serve MATLAB RAVEN.

## Building and publishing in one go

[`scripts/build_kegg_artefacts.py`](https://github.com/SysBioChalmers/raven-toolbox/blob/develop/scripts/README.md)
runs step 2 (+ step 3 with `--hmms`) and lays the output out as publishable,
version-prefixed assets: the core model files (reference model + KO/reaction
tables) bundled into `<version>_core.tar.gz`, and `<version>_<domain>.hmm.gz` per
domain. It also publishes `<version>_taxonomy.gz` --- the domain split plus the
source for
...
Then **publish** the artefacts and update the manifest --- see
[Artefact hosting & publishing](artefact_hosting.md) for the full workflow:
...
From then on raven-toolbox's `ensure_data` fetches and SHA256-verifies the
artefacts for Python end users automatically; MATLAB's `getKEGGModelForOrganism`
fetches directly from the same release.

## End-user paths

End users do **not** run the steps above --- the published artefacts are fetched
automatically on first use. The two toolboxes structure this differently: MATLAB's
single `getKEGGModelForOrganism` call handles download and per-organism query
together, where raven-toolbox splits it into a caching fetch (`ensure_data`,
under `~/.cache/raven-toolbox/data/kegg-<version>/`) plus two separate entry
points (pass an explicit `artefact_dir=`/`library=` to either to use your own
build instead):

- **Species already in KEGG** (`get_kegg_model_for_organism_from_artefacts`): no
  ...
  model (pass `taxonomy=`).
- **Organism not in KEGG** (`get_kegg_model_from_sequences`): `hmmscan`-es a
  ...
  `hmmscan`) --- Linux/macOS or WSL2 (see the OS matrix in
  [maintaining_binaries.md](maintaining_binaries.md#native-os-support-per-tool)).
  Tune assignment with `cutoff`, `min_score_ratio_ko`, `min_score_ratio_g`.
